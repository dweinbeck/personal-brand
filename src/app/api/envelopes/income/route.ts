...
import {
  createIncomeEntry,
  listIncomeEntriesForWeek,
} from "@/lib/envelopes/firestore";
import { incomeEntrySchema } from "@/lib/envelopes/types";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [access, entries] = await Promise.all([
      ...
      listIncomeEntriesForWeek(auth.uid, weekStart, weekEnd),
    ]);
    return Response.json({
      entries,
      billing: {
    ...
    console.error(
      "GET /api/envelopes/income error:",
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createIncomeEntry(auth.uid, parsed.data);
    return Response.json(result, { status: 201 });
  } catch (error) {
    console.error(
      "POST /api/envelopes/income error:",
      error instanceof Error ? error.message : "Unknown",
    );
    return Response.json(
