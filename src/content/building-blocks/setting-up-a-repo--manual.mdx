...
A repository is just a folder that stores your stuff, that Git tracks. Usually it contains code files, in whatever language you choose to work.  Often, it will also include images as well, especially when the repo is for a website or an app with a user interface.  So it's natural that the first step in a new prjoect is to create the folder which will eventually house the files we need for the final product.
