...
<task type="auto">
  ...
  <action>
    ...
    2. Create a placeholder OG image at `src/app/opengraph-image.png` (1200x630). Since we cannot generate images programmatically, create a simple 1x1 placeholder PNG file at this path and add a note in the summary that a proper 1200x630 OG image with Dan's name/title should replace it before production launch. The file must exist so Next.js auto-detects it via the opengraph-image convention. Alternatively, if a proper image cannot be created, skip the file and remove the images array from the OG config -- the OG tags will still work for title/description previews without an image.

    3. Update `src/app/layout.tsx` metadata export. Replace the existing simple metadata with:
