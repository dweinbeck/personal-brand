...
must_haves:
  truths:
    ...
    - "BrandTaxonomy schema validates brand data with colors, fonts, logos, assets, and identity sections"
    - "useJobStatus hook polls the job status endpoint with auth token and stops on terminal states"
    - "UrlSubmitForm submits a URL to the scrape API proxy and returns a job submission result"
    - "JobStatusIndicator shows queued/processing/succeeded/partial/failed states with appropriate visual indicators"
    - "BrandConfidenceBadge maps numeric 0-1 scores to high/medium/low visual tiers"
  artifacts:
...
# Phase 20 artifacts (will exist when this plan runs)
@src/lib/brand-scraper/types.ts
